...
HTTP_STATUS=$(curl -s -o /dev/null -w "%{http_code}" http://kijumbesmart.co.tz:2025)
if [ "$HTTP_STATUS" != "200" ]; then
  echo -e "${RED}Domain kijumbesmart.co.tz:2025 is not accessible (HTTP status: $HTTP_STATUS)${NC}"
...
certbot --nginx -d kijumbesmart.co.tz:2025 -d www.kijumbesmart.co.tz:2025

# Check if SSL setup was successful
if [ $? -eq 0 ]; then
  echo -e "${GREEN}SSL setup completed successfully!${NC}"
  echo -e "${GREEN}KijumbeSmart application is now accessible via HTTPS: https://kijumbesmart.co.tz:2025${NC}"
else
...
HTTPS_STATUS=$(curl -s -o /dev/null -w "%{http_code}" -k https://kijumbesmart.co.tz:2025)
